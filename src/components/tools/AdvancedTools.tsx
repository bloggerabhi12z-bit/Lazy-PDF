  ...
  async function run() {
    ...
    try {
      ...
      const sections = workbook.SheetNames.map((name) => ({
        heading: name,
        body: XLSX.utils.sheet_to_csv(workbook.Sheets[name]),
      }));
      downloadBlob(
        await createTextPdf(file.name, sections),
        `paperlane-${file.name.replace(/\.xlsx?$/i, "")}.pdf`,
      );
...
export function PowerPointToPdfTool() {
  ...
  async function run() {
    ...
    try {
      ...
      downloadBlob(
        await createTextPdf(file.name, sections),
        `paperlane-${file.name.replace(/\.pptx?$/i, "")}.pdf`,
      );
...
export function EpubToPdfTool() {
  ...
  async function run() {
    ...
    try {
      ...
      downloadBlob(
        await createTextPdf(file.name, sections),
        `paperlane-${file.name.replace(/\.epub$/i, "")}.pdf`,
      );
...
export function PdfToEpubTool() {
  ...
  async function run() {
    ...
    try {
      ...
      zip.file(
        "OEBPS/content.opf",
        `<?xml version="1.0"?><package version="3.0" xmlns="http://www.idpf.org/2007/opf" unique-identifier="id"><metadata xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:identifier id="id">paperlane</dc:identifier><dc:title>${file.name}</dc:title><dc:language>en</dc:language></metadata><manifest>${manifest}</manifest><spine>${spine}</spine></package>`,
      );
